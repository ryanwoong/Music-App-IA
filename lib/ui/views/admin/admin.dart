...
import 'package:pep/api/spotify_api.dart';
                    ...
                    children: <Widget>[
                      ...
                      Padding(
                        ...
                        child: Row(
                          children: <Widget>[
                            TextButton.icon(
                              icon: const Icon(Icons.search, color: constants.Colors.darkGrey,),
                              label: const Text("Select an audio file", style: constants.ThemeText.smallText),
                              onPressed: () async {
                                // Get the file and return FileStructure and update the state of the variables
                                FileStructure result = await FileService().pickFile();
                                // print("FILE: ${result.getSongFile()}");
                                // print("MESSAGE: ${result.getStatusMessage()}");
                                String? _statusMessage = result.getStatusMessage();
                                setState(() {
                                  if (_statusMessage == "Success") {
    ...
    child: Column(
      children: <Widget>[
        ...
        IconButton(
            ...
            onPressed: () async {
              error = await FileService().pickFile();
              print(error);
